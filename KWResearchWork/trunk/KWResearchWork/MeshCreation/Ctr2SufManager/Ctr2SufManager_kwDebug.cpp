...
void Ctr2SufManager::GetPolygonFace(vector<Int_Int_Pair> vecEdgeIn,vector<int>& vecVerOut)
{
	list<int> listVerOut;
	int iTotalEdgeNum=vecEdgeIn.size();
	listVerOut.push_back(vecEdgeIn.front().first);
	listVerOut.push_back(vecEdgeIn.front().second);
	vecEdgeIn.erase(vecEdgeIn.begin());
	while(listVerOut.size()!=iTotalEdgeNum)
	{
		for (vector<Int_Int_Pair>::iterator PairIter=vecEdgeIn.begin();PairIter!=vecEdgeIn.end();PairIter++)
		{
			int iStartInd=PairIter->first;
			int iEndInd=PairIter->second;
			if (iStartInd==listVerOut.back())
			{
				if (iEndInd!=listVerOut.front())
				{
					listVerOut.push_back(iEndInd);
				}
				vecEdgeIn.erase(PairIter);
				break;
			}
			else if (iEndInd==listVerOut.back())
			{
				if (iStartInd!=listVerOut.front())
				{
					listVerOut.push_back(iStartInd);
				}
				vecEdgeIn.erase(PairIter);
				break;
			}
			else if (iStartInd==listVerOut.front())
			{
				if (iEndInd!=listVerOut.back())
				{
					listVerOut.push_front(iEndInd);
				}
				vecEdgeIn.erase(PairIter);
				break;
			}
			else if (iEndInd==listVerOut.front())
			{
				if (iStartInd!=listVerOut.back())
				{
					listVerOut.push_front(iStartInd);
				}
				vecEdgeIn.erase(PairIter);
				break;
			}
	...
	vecVerOut.resize(listVerOut.size());
	copy(listVerOut.begin(),listVerOut.end(),vecVerOut.begin());
	//num of edges == num of vertices
	assert(iTotalEdgeNum==vecVerOut.size());
}
