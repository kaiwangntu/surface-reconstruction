...
void CMeshCreation::Init(CKWResearchWorkDoc* pDataIn)
{
	...
	this->bRenderRefPlane[0]=this->bRenderRefPlane[1]=this->bRenderRefPlane[2]=false;//true;
	this->bRenderCN=true;
	...
	this->iSurfReconstAlgorithm=ProgSurfReconstAlgo;//ProgSurfReconstAlgo//TaoJuSurfReconstAlgo
...
void CMeshCreation::ResetDrawingPlane()
{
...
void CMeshCreation::RotateDrawingPlane(int iRotOpt)
{
	//name of the selected plane
	int iPlane=this->pDoc->GetRBSelName();
	if ((iPlane<CREATION_PLANE_NAME_BEGIN)||(iPlane>CREATION_PLANE_NAME_END))
	...
	{
	case 0:
		StartPoint=CGAL::midpoint(PlaneBoundaryPoints[iPlane][0],PlaneBoundaryPoints[iPlane][1]);
		EndPoint=CGAL::midpoint(PlaneBoundaryPoints[iPlane][2],PlaneBoundaryPoints[iPlane][3]);
		dAngle=2;
		break;
	case 1:
		StartPoint=CGAL::midpoint(PlaneBoundaryPoints[iPlane][0],PlaneBoundaryPoints[iPlane][1]);
		...
		break;
	case 2:
		StartPoint=CGAL::midpoint(PlaneBoundaryPoints[iPlane][1],PlaneBoundaryPoints[iPlane][2]);
		EndPoint=CGAL::midpoint(PlaneBoundaryPoints[iPlane][3],PlaneBoundaryPoints[iPlane][0]);
		dAngle=2;
	    break;
	case 3:
		StartPoint=CGAL::midpoint(PlaneBoundaryPoints[iPlane][1],PlaneBoundaryPoints[iPlane][2]);
	...
	//compute rotated plane
	GeometryAlgorithm GeoCompute;
	GeoCompute.ComputeRotatedPlane(StartPoint,EndPoint,dAngle,RefPlane[iPlane]);
	//compute rotated plane boundary points
	for (int i=0;i<4;i++)
	{
		GeoCompute.ComputeRotatedPoint(StartPoint,EndPoint,dAngle,PlaneBoundaryPoints[iPlane][i]);
	}
...
void CMeshCreation::TranslateDrawingPlane(int iTransOpt)
{
	//name of the selected plane
	int iPlane=this->pDoc->GetRBSelName();
	if ((iPlane<CREATION_PLANE_NAME_BEGIN)||(iPlane>CREATION_PLANE_NAME_END))
	...
	//do not consider bounding box first
	//if (abs(dNewD)>=2)
	//{
	//	return;
	//}
	//compute the vector of the boundary points to move
	//treat it as a normalized vector by default
	Vector_3 PlaneNorm=this->RefPlane[iPlane].orthogonal_vector();
	Vector_3 BoundToMove=PlaneNorm*dOffset;

	//compute the new plane
	this->RefPlane[iPlane]=Plane_3(this->RefPlane[iPlane].a(),this->RefPlane[iPlane].b(),
							this->RefPlane[iPlane].c(),dNewD);
	//compute the plane boundary points
	for (int i=0;i<4;i++)
	{
		this->PlaneBoundaryPoints[iPlane][i]=this->PlaneBoundaryPoints[iPlane][i]+BoundToMove;
	}
...
void CMeshCreation::StopTransformDrawingPlane()
{
	//name of the selected plane
	int iPlane=this->pDoc->GetRBSelName();
	if ((iPlane<CREATION_PLANE_NAME_BEGIN)||(iPlane>CREATION_PLANE_NAME_END))
...
//	compute.ProcessCurverPoint(this->UserInput2DProfile,5.0);//20
	compute.ProcessCurverPoint(this->UserInput2DProfile,18.0);//15.0
...
//	if (NewProfile.size()==SAMPLE_POINT_NUM)//20
	if (!NewProfile.empty())
	{
		...
		GeometryAlgorithm::PlanarPolygonToXOY(NewProfile,NewProfile2D,this->RefPlane[this->iDrawingProfilePlane]);
...
void CMeshCreation::Input2DProfilePointTopoEdit(CPoint ProfilePoint)
{
	this->UserInput2DProfile.push_back(ProfilePoint);
}

void CMeshCreation::Convert2DProfileTo3DTopoEdit(int iType,KW_Mesh& Mesh,vector<double> vecMeshColor)
{
	if (this->UserInput2DProfile.size()<2)
	{
		return;
	}
	if (this->iSurfReconstAlgorithm!=ProgSurfReconstAlgo || this->kwcs2surf==NULL)
	{
		return;
	}
	...
	bool bResult=false;
	if (iType==CR_TOPO_JOIN)
	{
		bResult=this->kwcs2surf->SketchJoin(this->UserInput2DProfile,modelview,projection,viewport);
	}
