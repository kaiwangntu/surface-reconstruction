...
public:
	...
	//only return the result (intersect or not)
	//dScale: if the coordinates of the mesh have been scaled(mainly used in mesh creation operation)
	bool PaintingScrPointOnFrontalMeshQuick(KW_Mesh& Mesh,CPoint UserScrPoint,GLdouble* modelview,GLdouble* projection,GLint* viewport,double dScale=1.0);
	//get all the painted points
	//dScale: if the coordinates of the mesh have been scaled(mainly used in mesh creation operation)
	bool PaintingScrPointOnMesh(KW_Mesh& Mesh,CPoint UserScrPoint,vector<Point_3>& vecResultPoint,GLdouble* modelview,GLdouble* projection,GLint* viewport,double dScale=1.0);
