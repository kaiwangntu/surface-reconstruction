...
public:
	...
	//can handle only xoy,yoz and xoz or parallel cases
	//static void PlanarPolygonToXOY(std::vector<Point_3> InputPolygon,Polygon_2& OutputPolygon, int iPlaneType);
	//handle arbitrary planes
	static void PlanarPolygonToXOY(std::vector<Point_3> InputPolygon,Polygon_2& OutputPolygon, Plane_3 Plane);
	...
	static bool PlanarPolygonToXOYCCW(std::vector<Point_3>& InputPolygon,Polygon_2& OutputPolygon, Plane_3 Plane);
	...
	//handle arbitrary planes
	static void XOYPolygonTo3DPlanar(Polygon_2 InputPolygon,std::vector<Point_3>& OutputPolygon, Plane_3 Plane);
	...
	//handle arbitrary planes
	static bool IsPlanarPolygonCCW(std::vector<Point_3> InputPolygon, Plane_3 Plane);
	...
	//handle arbitrary planes
	static bool IsPlanarPolygonsIntersect(std::vector<Point_3> InputPolygon0,std::vector<Point_3> InputPolygon1, Plane_3 Plane);
	//2d version
	...
	//handle arbitrary planes
	static int PlanarPolygonsContained(std::vector<Point_3> InputPolygon0,std::vector<Point_3> InputPolygon1, Plane_3 Plane);
	//2d version
	...
	*/
	...
	//convert from double to float to avoid numerical issue in boolean operation in CGAL
	static void DbToFlForCGALBool(Polygon_2& InOutPoly);
	static void DbToFlForCGALBool(Polygon_with_holes_2& InOutPwh);

	//overide CGAL::do_intersect
	static bool CGAL_do_intersect(Polygon_2 Poly0,Polygon_2 Poly1);
	static bool CGAL_do_intersect(Polygon_with_holes_2 Pwh0,Polygon_with_holes_2 Pwh1);
	...
	//return 0 if no intersection; 1 if the intersection is a point; 2 if the intersection is a segment
	static int CGAL_intersection(Ray_3 RayIn,Triangle_3 TriIn,Point_3& PointOut,Segment_3& SegOut);
	//return 0 if no intersection; 1 if the intersection is a point; 2 if the intersection is a segment
	static int CGAL_intersection(Segment_3 SegIn,Triangle_3 TriIn,Point_3& PointOut,Segment_3& SegOut);
	////overide CGAL::projection
	//static Point_2 CGAL_projection(Line_2 LineIn,Point_2 PointIn);
	
	//print Polygon_2 or Polygon_with_holes_2 to file
	static void PrintCGALPolygon_2toFile(char* FileName,Polygon_2 PolyIn);
	//static void PrintCGALPwh_2toFile(Polygon_with_holes_2 Poly0);

	//remove duplicated points after CGAL_intersection operation
	//to avoid the bug in CGAL::intersection (two very close points exist)
	static void RemoveCGALInterDupPoints(Pwh_list_2& IntersectResult);
