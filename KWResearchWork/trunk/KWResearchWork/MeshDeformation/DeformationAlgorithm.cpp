												 ...
												 vector<Point_3>& vecDeformCurvePoint3d)
{
	...
	DBWindowWrite("HandlePoint: %d\n",vecHandlePoint.size());
	DBWindowWrite("HandleNB: %d\n",vecHandleNb.size());
	DBWindowWrite("ROI: %d\n",ROIVertices.size());
	DBWindowWrite("Anchor: %d\n",vecAnchorVertices.size());
	...
	DBWindowWrite("\n");
	DBWindowWrite("A size: %d*%d\n",vecvecLaplacianMatrix.size(),vecvecLaplacianMatrix.front().size());
	DBWindowWrite("B size: %d\n",LaplacianRightHandSide.front().size());
	DBWindowWrite("C size: %d*%d\n",LeftHandConstrainedMatrix.size(),LeftHandConstrainedMatrix.front().size());
	DBWindowWrite("D size: %d\n",RightHandConstrainedVector.front().size());
										...
										vector<Point_3>& vecDeformCurvePoint3d)
{
	...
	for (int iCurrent=0;iCurrent<=iIterNum;iCurrent++)
	{
		...
		if (iCurrent!=iIterNum)
		{
			...
			DBWindowWrite("time for computing rotation: %f\n",float(RotationEnd-RotationBegin));
		}
	}
}
...
{
	DBWindowWrite("num of handle: %d\n",vecHandleNb.size());
	DBWindowWrite("num of roi: %d\n",ROIVertices.size());
	DBWindowWrite("num of anchor: %d\n",vecAnchorVertices.size());
...
void CDeformationAlgorithm::GetInterpolationResult(vector<Point_3> vecDeformCurvePoint3d, 
												   ...
												   vector<Vertex_handle>& vecHandleNb)
{
	DBWindowWrite("Interpolation Result: \n");
														...
														vector<Point_3>& vecDeformCurvePoint3d)
{
	...
	for (int i=0;i<iInterpoNum;i++)
	{
		...
		OBJHandle::UnitizeCGALPolyhedron(Mesh,false,false);
		CString FileName;
		FileName.Format("%d%d%s",0,(i+1),"Linearflexible.obj");
		std::ofstream out(FileName,ios_base::out | ios_base::trunc);
		print_polyhedron_wavefront(out,Mesh);
	}
														...
														vector<Point_3>& vecDeformCurvePoint3d)
{
	...
	for (int iLambda=0;iLambda<(2+iInterpoNum);iLambda++)
	{
		...
		if (dLambda!=1.0)
		{
			...
			OBJHandle::UnitizeCGALPolyhedron(Mesh,false,false);
			CString FileName;
			FileName.Format("%d%d%s",0,iLambda,"Lambdaflexible.obj");
			std::ofstream out(FileName,ios_base::out | ios_base::trunc);
			print_polyhedron_wavefront(out,Mesh);
														   ...
														   vector<Point_3>& vecDeformCurvePoint3d)
{
	...
	for (int i=0;i<iInterpoNum;i++)
	{
		...
		OBJHandle::UnitizeCGALPolyhedron(Mesh,false,false);
		CString FileName;
		FileName.Format("%d%d%s",0,(i+1),"Laplacianflexible.obj");
		std::ofstream out(FileName,ios_base::out | ios_base::trunc);
		print_polyhedron_wavefront(out,Mesh);
...
//		GeneralMatrix CurrentRow=LpCo*vecCoeffMat.at(i)*Co2VecMat;
	...
	DBWindowWrite("Mi Time: %f\n",MiTime);
	DBWindowWrite("Build Time: %f\n",BuildTime);
	DBWindowWrite("Fill Time: %f\n",FillTime);
	DBWindowWrite("SubSys Time: %f\n",SubSysTime);
	DBWindowWrite("Multip1 Time: %f\n",Multip1);
	DBWindowWrite("Multip2 Time: %f\n",Multip2);
}
