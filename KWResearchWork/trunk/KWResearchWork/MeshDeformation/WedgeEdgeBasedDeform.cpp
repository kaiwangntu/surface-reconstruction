...
void CWedgeEdgeBasedDeform::BuildWedgeEdgeMesh(KW_Mesh& PrimalMesh,KW_Mesh& WedgeEdgeMesh)
{
	...
	do 
	{
		if (HI->GetEdgeIndex()!=iLastIndex)
		{
			...
			{
				DBWindowWrite("Error when building wedge edge mesh\n");
			}
		}
														  ...
														  SparseMatrix& VerToEdgeMatrix)
{
	...
	DBWindowWrite("edge roi num: %d\n",setEdgeROIIndex.size());
	DBWindowWrite("edge anchor num: %d\n",setEdgeAnchorIndex.size());
