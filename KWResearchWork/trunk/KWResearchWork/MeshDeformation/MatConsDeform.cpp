											   ...
											   vector<Vertex_handle>& vecAnchorVertices, vector<Point_3>& vecDeformCurvePoint3d,bool bTestIsoScale)
{
	...
	DBWindowWrite("related edge num: %d\n",ROIEdges.size());
...
void CMatConsDeform::ExportMat(KW_Mesh& Mesh,CString MatFileName)
{
	FILE* pfile=fopen(MatFileName,"w");
	if (pfile!=NULL)
	{
...
void CMatConsDeform::ImportMat(KW_Mesh& Mesh,CString MatFileName)
{
	FILE* pfile=fopen(MatFileName,"r");
	if (pfile!=NULL)
	{
		for (Vertex_iterator Vi=Mesh.vertices_begin();Vi!=Mesh.vertices_end();Vi++)	
		{
...
void CMatConsDeform::LearnMatFromSrc(int iType,KW_Mesh& Mesh,vector<CString> vecTmpName)
{
	...
	{
		DBWindowWrite("learning from model: %d\n",i);
		KW_Mesh TmpModel;
