										  ...
										  vector<Vertex_handle>& vecDualAnchorVertices)
{
	...
	DBWindowWrite("Dual Mesh size: %d\n",DualMesh.size_of_vertices());
	DBWindowWrite("Primal:  Handle: %d,ROI: %d,Anchor: %d\n",vecPrimalHandleNb.size(),PrimalROIVertices.size(),vecPrimalAnchorVertices.size());
	DBWindowWrite("Dual:    Handle: %d,ROI: %d,Anchor: %d",vecDualHandle.size(),DualROIVertices.size(),vecDualAnchorVertices.size());
}
											...
											vector<DualMeshVertexStruct>& DualMesh,int* pDualMeshVertexNum)
{
	...
	//}
	DBWindowWrite("Dual Mesh size: %d\n",DualMesh.size());
	DBWindowWrite("Handle: %d,ROI: %d,Anchor: %d",pDualMeshVertexNum[0],pDualMeshVertexNum[1],pDualMeshVertexNum[2]);
						 ...
						 vector<Vertex_handle>& DualROIVertices,vector<Vertex_handle>& vecDualAnchorVertices)
{
	...
	for (int iCurrent=0;iCurrent<=iIterNum;iCurrent++)
	{
		...
		clock_t RHS=clock();   
		DBWindowWrite("time for computing RHS: %f\n",float(RHS-IterationBegin));
		...
		DBWindowWrite("time for updating dual mesh: %f\n",float(IterationEnd-LSE));
	}
}
											 ...
											 bool bTestIsoScale/* =false */)
{
	...
	//}
	DBWindowWrite("related facet num: %d\n",ROIFacets.size());
	...
	DBWindowWrite("HandleNb Vertex num: %d\n",vecHandleNb.size());
	DBWindowWrite("ROI Vertex num: %d\n",ROIVertices.size());
	DBWindowWrite("Anchor Vertex num: %d\n",vecAnchorVertices.size());
	...
	DBWindowWrite("total time: %f\n",float(TotalEnd-TotalBegin));
}
