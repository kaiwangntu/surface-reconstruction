...
bool CPaintingOnMesh::PaintingScrPointOnFrontalMeshQuick(KW_Mesh& Mesh,CPoint UserScrPoint,GLdouble* modelview,GLdouble* projection,GLint* viewport,double dScale/* =1.0 */)
{
	//convert UserScrPoint from 2D windows coordinates to 3D opengl coordinates
	GLdouble  winX, winY, winZ; 
	GLdouble posX, posY, posZ; 
	...
	glReadPixels((int)winX, (int)winY, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &winZ); 
	gluUnProject(winX, winY, 0.0, modelview, projection, viewport, &posX, &posY, &posZ);
	Point_3 UserPoint(posX,posY,posZ);
	...
	for ( Facet_iterator j=Mesh.facets_begin(); j!=Mesh.facets_end(); j++)
	{
		...
		for (int i=0;i<3;i++)
		{
			TriVertex[i]=Point_3(TriVertex[i].x()*dScale,TriVertex[i].y()*dScale,TriVertex[i].z()*dScale);
		}
		Triangle_3 CurrentTri(TriVertex[0],TriVertex[1],TriVertex[2]);
		// note that since has_on method in CGAL is not accurate,so forbiden strictly!
		if (CGAL::do_intersect(CurrentTri,RayCameraUCP))
...
bool CPaintingOnMesh::PaintingScrPointOnMesh(KW_Mesh& Mesh,CPoint UserScrPoint,vector<Point_3>& vecResultPoint,
											 GLdouble* modelview,GLdouble* projection,GLint* viewport,double dScale/* =1.0 */)
{
	//convert UserScrPoint from 2D windows coordinates to 3D opengl coordinates
	GLdouble  winX, winY, winZ; 
	GLdouble posX, posY, posZ; 
	...
	for ( Facet_iterator j=Mesh.facets_begin(); j!=Mesh.facets_end(); j++)
	{
		...
		if (CGAL::do_intersect(CurrentTri,RayCameraUCP))
		{
			...
			if (CGAL::assign(CurrentIP, result)) 
			{
				vecResultPoint.push_back(CurrentIP);
			}
		}
	}
	if (vecResultPoint.empty())
	{
		return false;
	}
	//scale back
	...
	{
		vecResultPoint.at(i)=Point_3(vecResultPoint.at(i).x()/dScale,vecResultPoint.at(i).y()/dScale,
			vecResultPoint.at(i).z()/dScale);
	}
	return true;
}
